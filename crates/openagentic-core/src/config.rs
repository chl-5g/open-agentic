...
impl Default for LongTermMemoryConfig {
    fn default() -> Self {
        Self {
            ...
            collection: "openagentic_memories".to_string(),
...
impl Default for MilvusConfig {
    fn default() -> Self {
        Self {
            ...
            collection: "openagentic_vectors".to_string(),
            dimension: Some(1536),
        }
    }
}
...
impl Default for AgentDefaults {
    fn default() -> Self {
        Self {
            workspace: PathBuf::from("~/.open-agentic/workspace"),
        }
    }
}

// ============== 工作区配置 (迁移到 openagentic-memory) ==============
...
impl Config {
    ...
    pub fn from_file(path: &std::path::Path) -> crate::Result<Self> {
        let content = std::fs::read_to_string(path)
            .map_err(|e| crate::OpenAgenticError::Config(format!("读取配置文件失败: {}", e)))?;

        let config: Config = serde_json::from_str(&content)
            .map_err(|e| crate::OpenAgenticError::Config(format!("解析配置文件失败: {}", e)))?;
    ...
    pub fn save(&self, path: &std::path::Path) -> crate::Result<()> {
        let content = serde_json::to_string_pretty(self)
            .map_err(|e| crate::OpenAgenticError::Config(format!("序列化配置失败: {}", e)))?;

        std::fs::write(path, content)
            .map_err(|e| crate::OpenAgenticError::Config(format!("写入配置文件失败: {}", e)))?;
