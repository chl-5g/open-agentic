...
impl UnifiedConfig {
    pub fn load(path: &PathBuf) -> crate::Result<Self> {
        ...
        let content = fs::read_to_string(path)
            .map_err(|e| crate::OpenAgenticError::Config(format!("读取配置失败: {}", e)))?;

        let config: Self = serde_json::from_str(&content)
            .map_err(|e| crate::OpenAgenticError::Config(format!("解析配置失败: {}", e)))?;
    ...
    pub fn save(&self, path: &PathBuf) -> crate::Result<()> {
        if let Some(parent) = path.parent() {
            fs::create_dir_all(parent)
                .map_err(|e| crate::OpenAgenticError::Config(format!("创建目录失败: {}", e)))?;
        }
        ...
        fs::write(path, content)
            .map_err(|e| crate::OpenAgenticError::Config(format!("写入配置失败: {}", e)))?;
