...
impl ToolPort for SkillToolAdapter {
    async fn execute(
        ...
        let skill = self.registry
            ...
            .await
            .ok_or_else(|| OpenClawError::Tool(format!("Skill not found: {}", tool_name)))?;

        let compiled = self.registry
            .get_compiled_skill(&skill.id)
            .await
            .ok_or_else(|| OpenClawError::Tool(format!("Compiled skill not found: {}", skill.id)))?;
    ...
    async fn execute_with_sandbox(
        ...
        if enable_sandbox {
            ...
            Err(OpenClawError::Execution("Sandbox disabled".to_string()))
        }
    }
