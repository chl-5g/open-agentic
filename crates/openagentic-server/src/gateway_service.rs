...
use openagentic_vector::init_all_factories;
...
impl Gateway {
    ...
    pub async fn start(&self) -> openagentic_core::Result<()> {
        ...
        let addr: SocketAddr = format!("{}:{}", self.config.server.host, self.config.server.port)
            .parse()
            .map_err(|e| openagentic_core::OpenAgenticError::Config(format!("Invalid address: {}", e)))?;

        tracing::info!("OpenAgentic Gateway starting on {}", addr);
        tracing::info!(
        ...
        let listener = tokio::net::TcpListener::bind(addr)
            .await
            .map_err(|e| openagentic_core::OpenAgenticError::Config(format!("绑定地址失败: {}", e)))?;

        axum::serve(listener, app)
            .await
            .map_err(|e| openagentic_core::OpenAgenticError::Unknown(e.to_string()))?;

        Ok(())
    }

    async fn init_voice_service(&self) -> openagentic_core::Result<()> {
