...
#[derive(Parser)]
#[command(name = "open-agentic")]
#[command(about = "OpenAgentic - Your personal AI assistant (Rust implementation)", long_about = None)]
#[command(version)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
...
enum Commands {
    ...
    Init {
        ...
        #[arg(short, long, default_value = "~/.open-agentic/openagentic.json")]
        config: String,
    },
...
async fn main() -> Result<()> {
    ...
    tracing_subscriber::registry()
        .with(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "openagentic=debug,info".into()),
        )
    ...
    match cli.command {
        Commands::Gateway {
            ...
            openagentic_device::init_device(true).await?;
